...
<!--              </template>-->
              ...
              <v-toolbar-title>{{ $t('Person')}} : {{totalElement}}</v-toolbar-title>
                 ...
                 ></v-divider>
                 <br>
                 <v-icon class="excel" color="#8AD86E"  @click="exportcompanionToExcel(page)">fa-thin fa-file-excel</v-icon>
                 <v-divider
                     class="mx-4"
                     inset
                     vertical
...
/*    { text: 'Seree', value: 'seree' },
  ...
  methods: {
    ...
    exportPDF(page) {
      ...
      const tableData = this.companion.map((dessert, index) => {
    ...
    exportcompanionToExcel(page) {
      let count=(page-1)*10 + 1;
      const excelData =this.companion.map(disserts=>({
        "№":count++,
    ...
    editItem(item) {
      this.companion.push(item)
      this.editedIndex = this.companion.indexOf(item)
    ...
    async deleteItemConfirm() {
      const districtResponse = await axios.delete('companion/delete/'+this.deleteId, {headers: {'authorization': this.token}})
      this.companion.splice(this.editedIndex, 1)
      this.closeDelete()
    },
    ...
    async save() {
        ...
        await axios.put('companion/edit/'+this.editedItem.id,  this.editedItem, {headers: {'authorization': this.token}})
        Object.assign(this.companion[this.editedIndex], this.editedItem)
        this.close()
        this.nextperson()
       }
       else{
       ...
       await axios.post('companion/add', this.editedItem, {headers: {'authorization': this.token}})
       this.close()
       this.nextperson()
      }
    }
