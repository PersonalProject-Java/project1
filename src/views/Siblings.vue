...
<!--              </template>-->
              ...
              <v-toolbar-title>{{ $t('Person')}} : {{totalElement}}</v-toolbar-title>
                 ...
                 ></v-divider>
                 <br>
                 <v-icon class="excel" color="#8AD86E"  @click="exportsiblingsToExcel(page)">fa-thin fa-file-excel</v-icon>
                 <v-divider
                     class="mx-4"
                     inset
                     vertical
...
/*    { text: 'Seree', value: 'seree' },
  ...
  methods: {
    ...
    exportPDF(page) {
      ...
      const tableData = this.siblings.map((dessert, index) => {
    ...
    exportsiblingsToExcel(page) {
      let count=(page-1)*10 + 1;
      const excelData =this.siblings.map(disserts=>({
        "№":count++,
    ...
    async nextperson() {
      ...
      if (this.search!== '' && this.search.length > 3 && response.data.length!==0){
        this.siblings=response.data.content
    ...
    editItem(item) {
      ...
      this.editedIndex = this.siblings.indexOf(item)
    ...
    async deleteItemConfirm() {
      const districtResponse = await axios.delete('siblings/delete/'+this.deleteId, {headers: {'authorization': this.token}})
      this.siblings.splice(this.editedIndex, 1)
      this.closeDelete()
    },
    ...
    async save() {
        ...
        await axios.put('siblings/edit/'+this.editedItem.id,  this.editedItem, {headers: {'authorization': this.token}})
        Object.assign(this.siblings[this.editedIndex], this.editedItem)
        this.close()
        this.nextperson()
       }
       else{
       ...
       await axios.post('siblings/add', this.editedItem, {headers: {'authorization': this.token}})
       this.close()
       this.nextperson()
      }
    }
