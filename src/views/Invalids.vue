...
<!--              </template>-->
              ...
              <v-toolbar-title>{{ $t('Person')}} : {{totalElement}}</v-toolbar-title>
                 ...
                 ></v-divider>
                 <br>
                 <v-icon class="excel" color="#8AD86E"  @click="exportinvalidsToExcel(page)">fa-thin fa-file-excel</v-icon>
                 <v-divider
                     class="mx-4"
                     inset
                     vertical
...
/*    { text: 'Seree', value: 'seree' },
  ...
  methods: {
    ...
    exportPDF(page) {
      ...
      const tableData = this.invalids.map((dessert, index) => {
    ...
    exportinvalidsToExcel(page) {
      let count=(page-1)*10 + 1;
      const excelData =this.invalids.map(disserts=>({
        "№":count++,
    ...
    editItem(item) {
      this.invalids.push(item)
      this.editedIndex = this.invalids.indexOf(item)
    ...
    async deleteItemConfirm() {
      const districtResponse = await axios.delete('invalids/delete/'+this.deleteId, {headers: {'authorization': this.token}})
      this.invalids.splice(this.editedIndex, 1)
      this.closeDelete()
    },
    ...
    async save() {
        ...
        await axios.put('invalids/edit/'+this.editedItem.id,  this.editedItem, {headers: {'authorization': this.token}})
        Object.assign(this.invalids[this.editedIndex], this.editedItem)
        this.close()
        this.nextperson()
       }
       else{
       ...
       await axios.post('invalids/add', this.editedItem, {headers: {'authorization': this.token}})
       this.close()
       this.nextperson()
      }
    }
