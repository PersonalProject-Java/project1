...
<!--              </template>-->
              ...
              <v-toolbar-title>{{ $t('Person')}} : {{totalElement}}</v-toolbar-title>
                 ...
                 ></v-divider>
                 <br>
                 <v-icon class="excel" color="#8AD86E"  @click="exportwareorToExcel(page)">fa-thin fa-file-excel</v-icon>
                 <v-divider
                     class="mx-4"
                     inset
                     vertical
...
/*    { text: 'Seree', value: 'seree' },
  ...
  methods: {
    ...
    exportPDF(page) {
      ...
      const tableData = this.wareor.map((dessert, index) => {
    ...
    exportwareorToExcel(page) {
      let count=(page-1)*10 + 1;
      const excelData =this.wareor.map(disserts=>({
        "№":count++,
    ...
    editItem(item) {
      this.wareor.push(item)
      this.editedIndex = this.wareor.indexOf(item)
    ...
    async deleteItemConfirm() {
      const districtResponse = await axios.delete('wareor/delete/'+this.deleteId, {headers: {'authorization': this.token}})
      this.wareor.splice(this.editedIndex, 1)
      this.closeDelete()
    },
    ...
    async save() {
        ...
        await axios.put('wareor/edit/'+this.editedItem.id,  this.editedItem, {headers: {'authorization': this.token}})
        Object.assign(this.wareor[this.editedIndex], this.editedItem)
        this.close()
        this.nextperson()
       }
       else{
       ...
       await axios.post('wareor/add', this.editedItem, {headers: {'authorization': this.token}})
       this.close()
       this.nextperson()
      }
    }
