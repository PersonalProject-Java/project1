...
<!--              </template>-->
              ...
              <v-toolbar-title>{{ $t('Person')}} : {{totalElement}}</v-toolbar-title>
                 ...
                 ></v-divider>
                 <br>
                 <v-icon class="excel" color="#8AD86E"  @click="exportothersToExcel(page)">fa-thin fa-file-excel</v-icon>
                 <v-divider
                     class="mx-4"
                     inset
                     vertical
...
/*    { text: 'Seree', value: 'seree' },
  ...
  methods: {
    ...
    exportPDF(page) {
      ...
      const tableData = this.others.map((dessert, index) => {
    ...
    exportothersToExcel(page) {
      let count=(page-1)*10 + 1;
      const excelData =this.others.map(disserts=>({
        "№":count++,
    ...
    async deleteItemConfirm() {
      const districtResponse = await axios.delete('others/delete/'+this.deleteId, {headers: {'authorization': this.token}})
    ...
    async save() {
        ...
        await axios.put('others/edit/'+this.editedItem.id,  this.editedItem, {headers: {'authorization': this.token}})
       ...
       await axios.post('others/add', this.editedItem, {headers: {'authorization': this.token}})
       this.close()
       this.nextperson()
      }
    }
