...
<!--              </template>-->
              ...
              <v-toolbar-title>{{ $t('Person')}} : {{totalElement}}</v-toolbar-title>
                 ...
                 ></v-divider>
                 <br>
                 <v-icon class="excel" color="#8AD86E"  @click="exportawardeesToExcel(page)">fa-thin fa-file-excel</v-icon>
                 <v-divider
                     class="mx-4"
                     inset
                     vertical
...
/*    { text: 'Seree', value: 'seree' },
  ...
  methods: {
    ...
    exportPDF(page) {
      ...
      const tableData = this.awardees.map((dessert, index) => {
    ...
    exportawardeesToExcel(page) {
      let count=(page-1)*10 + 1;
      const excelData =this.awardees.map(disserts=>({
        "№":count++,
    ...
    editItem(item) {
      this.awardees.push(item)
      this.editedIndex = this.awardees.indexOf(item)
    ...
    async deleteItemConfirm() {
      const districtResponse = await axios.delete('awardees/delete/'+this.deleteId, {headers: {'authorization': this.token}})
      this.awardees.splice(this.editedIndex, 1)
      this.closeDelete()
    },
    ...
    async save() {
        ...
        await axios.put('awardees/edit/'+this.editedItem.id,  this.editedItem, {headers: {'authorization': this.token}})
        Object.assign(this.awardees[this.editedIndex], this.editedItem)
        this.close()
        this.nextperson()
       }
       else{
       ...
       await axios.post('awardees/add', this.editedItem, {headers: {'authorization': this.token}})
       this.close()
       this.nextperson()
      }
    }
